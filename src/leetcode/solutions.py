...
class LeetCodeSolutionScraper:
    ...
    def _get_solution_links(self) -> List[str]:
        """
        Collect solution detail page URLs from the solutions listing.
        LeetCode renders solution cards as React components — try multiple strategies.
        """
        links: List[str] = []
        seen: set = set()

        # Wait a bit extra for React to finish rendering
        self.page.wait_for_timeout(2_000)

        # Strategy 1: <a> tags with /solutions/ in href
        try:
            ...
            logger.debug(f"Strategy 1 (a href) failed: {e}")

        if links:
            logger.debug(f"Found {len(links)} solution links via <a> tags")
            return links

        # Strategy 2: JS — collect all href attributes matching /problems/.../solutions/
        try:
            js_links = self.page.evaluate(
                """() => {
                    ...
                    document.querySelectorAll('a').forEach(a => {
                        ...
                        if (href.includes('/solutions/') && !href.endsWith('/solutions/') && !href.endsWith('/solutions')) {
                            const full = href.startsWith('http') ? href : 'https://leetcode.com' + href;
            ...
            if js_links:
                logger.debug(f"Found {len(js_links)} solution links via JS")
                return js_links
        except Exception as e:
            logger.debug(f"Strategy 2 (JS a tags) failed: {e}")

        # Strategy 3: extract problem slug from URL and build solution URLs from
        # any clickable card elements that have title text
        try:
            current_url = self.page.url
            m = re.match(r'https://leetcode\.com/problems/([^/?#]+)', current_url)
            if m:
                slug = m.group(1)
                # Look for article/div elements that could be solution cards
                cards = self.page.evaluate(
                    """() => {
                        const items = [];
                        // Try to find solution article ids from data attributes or links
                        document.querySelectorAll('[data-row-index], [class*="solution"], article').forEach(el => {
                            const link = el.querySelector('a');
                            if (link) {
                                const href = link.getAttribute('href') || '';
                                if (href) items.push(href.startsWith('http') ? href : 'https://leetcode.com' + href);
                            }
                        });
                        return items;
                    }"""
                ...
                if cards:
                    logger.debug(f"Found {len(cards)} links via card strategy")
                    return list(dict.fromkeys(cards))  # dedupe preserving order
        except Exception as e:
            logger.debug(f"Strategy 3 (cards) failed: {e}")
